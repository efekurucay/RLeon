...
struct ContentView: View {
    ...
    private enum SidebarSection: String, CaseIterable, Identifiable, Hashable {
        ...
        var title: String {
            ...
            case .llm: return "LLM"
            case .mediaBatch: return "Media Batch"
        ...
        var symbol: String {
            ...
            case .mediaBatch: return "square.stack.3d.up.fill"
    ...
    var body: some View {
        NavigationSplitView {
            List(selection: $sidebarSelection) {
                ...
                Section("Tools") {
                    Label(SidebarSection.mediaBatch.title, systemImage: SidebarSection.mediaBatch.symbol)
                        .tag(SidebarSection.mediaBatch)
                }
                Section("Shortcut") {
    ...
    private var homeView: some View {
        VStack(alignment: .leading, spacing: 20) {
            RLeonSectionHeader("RLeon", subtitle: "On-device speech, OCR, Ollama, and utility tools in one window.")
            LazyVGrid(columns: [GridItem(.adaptive(minimum: 220), spacing: 16)], spacing: 16) {
                ...
                homeCard(title: "Media Batch", subtitle: "Gemini batch file processing", icon: "square.stack.3d.up.fill", section: .mediaBatch)
    ...
    private var mediaBatchTab: some View {
        MediaBatchToolView()
    }
    ...
    private var settingsTab: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                RLeonSectionHeader("Settings", subtitle: "Tools, permissions, hotkey, environment variables, and version.")
                ToolsSettingsSection(toolStore: toolStore)
                EnvironmentVariablesSettingsSection()
    ...
    private var fnPushToTalkStatusTab: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                if let transcript = fnCoordinator.lastDictationOnlyTranscript, !transcript.isEmpty {
                    Text("Last dictation")
                        .font(.subheadline.weight(.semibold))
                    if let focusMessage = fnCoordinator.lastDictationFocusMessage {
                        Text(FocusedTextInsertion.localizedUserMessage(for: focusMessage))
                            .font(.caption2)
                            .foregroundStyle(.secondary)
                        Text("Code: \(focusMessage)")
                            .font(.caption2)
                            .foregroundStyle(.tertiary)
                    }
                    Text(transcript)
...
struct ContentView_Previews: PreviewProvider {
    static var previews: some View {
        ...
        ContentView()
            ...
            .environmentObject(app.mediaBatchTool)
            .environmentObject(app.appEnvironment)
    }
}
